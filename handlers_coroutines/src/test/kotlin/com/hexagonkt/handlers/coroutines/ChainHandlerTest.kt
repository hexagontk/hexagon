...
import com.hexagonkt.handlers.coroutines.HandlerTest.Companion.process
...
internal class ChainHandlerTest {
    ...
    @Test fun `Only the first 'on' handler is processed`() = runBlocking {
    ...
    @Test fun `Build a nested chain of handlers`() = runBlocking {
        var flags = listOf(true, true, true, true)
    ...
    @Test fun `Chains of handlers pass the correct handler to context`() = runBlocking {
    ...
    @Test fun `Process event with chain of handlers ('delay' ignored)`() = runBlocking {
        val t = time(10) {
            launch {
    ...
    @Test fun `Before and after handlers are called in order`() = runBlocking {
    ...
    @Test fun `Filters allow passing and halting`() = runBlocking {
    ...
    @Test fun `Chained handlers are executed as blocks`() = runBlocking {
        val chain = ChainHandler<String>(
    ...
    @Test fun `Many chained handlers are processed properly`() = runBlocking {
        val chain = ChainHandler<String>(
            BeforeHandler { it.appendText("<B0>") },
    ...
    @Test fun `Chained handlers are executed as blocks bug`() = runBlocking {
        val chain = ChainHandler<String>(
            BeforeHandler { it.appendText("<B0>") },
            ChainHandler({ it.hasLetters('a', 'b', 'c') },
                BeforeHandler { it.appendText("<B1>")},
    ...
    @Test fun `Exceptions don't prevent handlers execution`() = runBlocking {
        val chain = ChainHandler<String>(
    ...
    @Test fun `Exceptions don't prevent handlers execution in after`() = runBlocking {
    ...
    @Test fun `Exceptions don't prevent handlers execution in filters`() = runBlocking {
        val chain = ChainHandler<String>(
    ...
    @Test fun `Context attributes are passed correctly`() = runBlocking {
        val chain = ChainHandler<String>(
            BeforeHandler { error("Fail") },
