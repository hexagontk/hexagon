...
internal class HandlerTest {
    ...
    internal companion object {
        suspend fun <T : Any> Handler<T>.process(event: T): T =
    ...
    @Test fun `When a callback completes, then the result is returned`() = runBlocking {
        val filter = FilterHandler<String> { it.with(it.event + ":OK") }
        assertEquals("Message:OK", filter.process("Message"))
    }

    @Test fun `When a callback fail, then the context contains the exception`() = runBlocking {
        listOf<Handler<Unit>>(
