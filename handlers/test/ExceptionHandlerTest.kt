...
internal class ExceptionHandlerTest {
    ...
    @Test fun `Exceptions are cleared properly`() {
        ...
        ChainHandler(
            ExceptionHandler<String, Exception>(Exception::class) { _, e -> throw e },
            OnHandler { error("Error") }
        )
        .process(EventContext("test", { true }))
        .let {
        ...
        ChainHandler(
            ExceptionHandler<String, Exception>(Exception::class) { c, e ->
                c.with("ok")
                throw e
            },
            ExceptionHandler(Exception::class) { _, _ -> error("Fail") },
            OnHandler { error("Error") }
        )
        .process(EventContext("test", { true }))
        .let {
