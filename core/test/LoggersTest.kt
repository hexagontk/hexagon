...
internal class LoggersTest {
    ...
    @Test fun loggerUsage() {
        ...
        instanceLogger.trace { "Message only evaluated if trace enabled" }
        instanceLogger.debug { "Message only evaluated if debug enabled" }
        instanceLogger.warn { "Message only evaluated if warn enabled" }
        instanceLogger.info { "Message only evaluated if info enabled" }

        instanceLogger.warn(exception) { "Warning with exception" }
        instanceLogger.error(exception) { "Error message with exception" }
        instanceLogger.warn(exception)
        instanceLogger.error(exception)
        instanceLogger.error { "Error without an exception" }
    }
