...
abstract class SseTest(
    ...
    private fun checkRequest(publisher: () -> Publisher<ServerEvent>) {
        ...
        clientPublisher.subscribe(object : Flow.Subscriber<ServerEvent> {
            ...
            override fun onNext(item: ServerEvent) {
                assertEqualEvents(pendingEvents.first(), item.info())
    ...
    private fun assertEqualEvents(a: ServerEvent, b: ServerEvent) {
        if (a.javaClass != b.javaClass) assert(false)
        if (a.event != b.event) assert(false)
        if (a.data != b.data) assert(false)
        if (a.id != b.id) assert(false)
        if (a.retry != b.retry) assert(false)
    }
}
