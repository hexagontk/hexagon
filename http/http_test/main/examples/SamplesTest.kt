...
abstract class SamplesTest(
    ...
    @Test fun serverCreation() {
        ...
        /*
         * You may skip the settings parameter and the defaults will be used
         */
        val runningServer = serve(serverAdapter(), path, settings.with(bindPort = 0))
    ...
    @Test open fun callbacks() {
        ...
        server.use { s ->
            ...
            HttpClient(clientAdapter(), HttpClientSettings(s.binding)).use {
                ...
                assertEquals(OK_200, it.get("/response").status)
    ...
    @Test fun files() {
        ...
        server.use { s ->
            HttpClient(clientAdapter(), HttpClientSettings(s.binding)).use {
                ...
                assertEquals(ContentType(TEXT_HTML).text, index.contentType?.text)
                val file = it.get("/web/file.css")
                assertEquals(OK_200, file.status)
                assertEquals(ContentType(TEXT_CSS).text, file.contentType?.text)
    ...
    @Test fun test() {
        ...
        val serverSettings = HttpServerSettings(bindAddress, 0)
