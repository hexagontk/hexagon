...
abstract class WebSocketsTest(
    ...
    @Test fun `WebSockets client check start and stop states`() {
        val settings = clientSettings.with(baseUri = URI("https://localhost:9999"))
    ...
    @Test fun `Serve WS works properly`() {
        wsTest(serverSettings.with(bindPort = 0), clientSettings)
    }

    @Test
    @DisabledOnOs(WINDOWS) // TODO There are problems with certificates in Windows
    fun `Serve WSS works properly`() {
        wsTest(http2ServerSettings.with(protocol = HTTPS), clientSettings)
    }
    ...
    private fun wsTest(
        ...
        val client = HttpClient(clientAdapter(), clientSettings.with(baseUri = server.binding))
        client.start()
