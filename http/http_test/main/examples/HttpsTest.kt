...
abstract class HttpsTest(
    ...
    fun `Serve HTTPS works properly`() {
        ...
        val server = serve(serverAdapter(), handler, http2ServerSettings.with(protocol = HTTPS))
    ...
    fun `Serve insecure HTTPS example`() {
        ...
        val serverSettings = serverSettings.with(
            bindPort = 0,
            protocol = HTTPS,
        ...
        val client = HttpClient(clientAdapter(), clientSettings.with(baseUri = base))
        client.start()
        client.get("/hello").apply {
            assertEquals("Hello World!", body)
        }
        ...
        assertFails {
            ...
            HttpClient(adapter, noTrustStore.with(baseUri = base)).use {
                it.start()
                it.get("/hello")
            }
        }
        ...
        assertFails {
            val adapter = clientAdapter()
            val contextPath1 = URI("https://127.0.0.1:${server.runtimePort}")
            HttpClient(adapter, clientSettings.with(baseUri = contextPath1)).use {
                it.start()
                it.get("/hello")
            }
        }
        ...
        val insecureClient = HttpClient(
            clientAdapter(),
            clientSettings.with(baseUri = base, insecure = true, sslSettings = SslSettings())
        )
        ...
        val settings = clientSettings.with(
            baseUri = base,
            insecure = false,
            sslSettings = SslSettings()
        )
