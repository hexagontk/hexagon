...
internal class HttpClientToolTest {
    ...
    @Test fun `Check all HTTP methods`() {
        server.path {
            before("*") {
                val headersText = request.headers.joinToString { "${it.name}=${it.value}" }
                ok("$method $path $headersText", contentType = text)
            }

            on(setOf(PUT, POST), "/bye/mike/{id}") {
                val headersText = request.headers.joinToString { "${it.name}=${it.value}" }
                ok("$method $path ${request.bodyString()} $headersText", contentType = text)
            }
        }
        ...
        client.get("/hello/mike/{id}", params).assertBody("GET /hello/mike/9", headers)
        client.put("/hello/mike/{id}", params).assertBody("PUT /hello/mike/9", headers)
        client.post("/hello/mike/{id}", params).assertBody("POST /hello/mike/9", headers)
        client.options("/hello/mike/{id}", params).assertBody("OPTIONS /hello/mike/9", headers)
        client.delete("/hello/mike/{id}", params).assertBody("DELETE /hello/mike/9", headers)
        client.patch("/hello/mike/{id}", params).assertBody("PATCH /hello/mike/9", headers)
        client.trace("/hello/mike/{id}", params).assertBody("TRACE /hello/mike/9", headers)

        client.put("/bye/mike/{id}", params) { "putLambdaBody" }
            .assertBody("PUT /bye/mike/9 putLambdaBody", headers)
        client.post("/bye/mike/{id}", params) { "postLambdaBody" }
