...
import com.hexagonkt.http.handlers.async.*
...
abstract class SamplesTest(
    ...
    @Test fun routesCreation() {
        val server = serve(serverAdapter()) {
            // routesCreation
            get("/hello") { ok("Get greeting").done() }
            put("/hello") { ok("Put greeting").done() }
            post("/hello") { ok("Post greeting").done() }

            on(ALL - GET - PUT - POST, "/hello") { ok("Fallback if HTTP verb was not used before").done() }

            on(status = NOT_FOUND_404) { ok("Get at '/' if no route matched before").done() }
            // routesCreation
        }
    ...
    @Test fun routeGroups() {
        val server = serve(serverAdapter()) {
            // routeGroups
            path("/nested") {
                get("/hello") { ok("Greeting").done() }

                path("/secondLevel") {
                    get("/hello") { ok("Second level greeting").done() }
                }

                get { ok("Get at '/nested'").done() }
            }
            // routeGroups
        }
    ...
    @Test fun routers() {
        ...
        fun personRouter(kind: String) = path {
            get { ok("Get $kind").done() }
            put { ok("Put $kind").done() }
            post { ok("Post $kind").done() }
        }
    ...
    @Test fun callbacks() {
        val server = HttpServer(serverAdapter()) {
            ...
            post("/file") {
                ...
                ok(filePart.body).done()
            }
            // callbackFile

            // callbackRedirect
            get("/redirect") {
                send(FOUND_302, "/call").done() // browser redirect to /call
            }
            // callbackRedirect
            ...
            get("/halt") {
                ...
                internalServerError().done()        // Halt with status 500
            }
            // callbackHalt
        }
    ...
    @Test fun filters() {
        ...
        val server = HttpServer(serverAdapter()) {
            // filters
            on("/*") { send(headers = response.headers + Header("b-all", "true")).done() }

            on("/filters/*") { send(headers = response.headers + Header("b-filters", "true")).done() }
            get("/filters/route") { ok("filters route").done() }
            after("/filters/*") { send(headers = response.headers + Header("a-filters", "true")).done() }

            get("/filters") { ok("filters").done() }

            path("/nested") {
                on("*") { send(headers = response.headers + Header("b-nested", "true")).done() }
                on { send(headers = response.headers + Header("b-nested-2", "true")).done() }
                get("/filters") { ok("nested filters").done() }
                get("/halted") { send(HttpStatus(499), "halted").done() }
                get { ok("nested also").done() }
                after("*") { send(headers = response.headers + Header("a-nested", "true")).done() }
            }

            after("/*") { send(headers = response.headers + Header("a-all", "true")).done() }
            // filters
        }
    ...
    @Test fun errors() {
        ...
        val server = serve(serverAdapter()) {
            ...
            get("/errors") { send(HttpStatus(512)).done() }
            ...
            get("/codedExceptions") { send(HttpStatus(509), "code").done() }

            on(pattern = "*", status = HttpStatus(509)) {
                send(HttpStatus(599)).done()
            }
            on(pattern = "*", exception = IllegalStateException::class) {
                send(HTTP_VERSION_NOT_SUPPORTED_505, exception?.message ?: "empty").done()
            }
            // exceptions
        }
    ...
    @Test fun files() {
        val server = serve(serverAdapter()) {
            // files
            get("/web/file.txt") { ok("It matches this route and won't search for the file").done() }
    ...
    @Test fun mockRequest() {
        ...
        val callback: HttpCallback = {
            ...
            ok("Callback result $fakeAttribute $fakeHeader").done()
        }
        ...
        assertEquals("Callback result attribute header", resultContext.join().response.bodyString())
        ...
        assertEquals(NOT_FOUND_404, notFound.join().status)
        assertEquals(OK_200, ok.join().status)
        assertEquals("Callback result null null", ok.join().response.bodyString())
        // mockRequest
    }
}
