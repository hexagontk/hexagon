...
import com.hexagonkt.http.server.async.callbacks.CorsCallback
...
abstract class CorsTest(
    ...
    private fun HandlerBuilder.corsPath(path: String, cors: CorsCallback) {
        path(path) {
            ...
            get("/path") { ok(method.toString()).done() }
            post("/path") { ok(method.toString()).done() }
            put("/path") { ok(method.toString()).done() }
            delete("/path") { ok(method.toString()).done() }
            get { ok(method.toString()).done() }
            post { ok(method.toString()).done() }
            put { ok(method.toString()).done() }
            delete { ok(method.toString()).done() }
        }
    }
    // cors
    ...
    @Test fun `Allowed origin is returned properly`() {
        listOf(
            ...
            assert(it.headers["vary"]?.string()?.contains("Origin")?.not() ?: true)
        }
    }
    ...
    @Test fun `Simple CORS request`() {
        ...
        assert(result.headers["vary"]?.string()?.contains("Origin") ?: false)
