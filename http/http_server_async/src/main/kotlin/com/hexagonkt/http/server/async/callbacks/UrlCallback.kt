...
class UrlCallback(private val url: URL) : (HttpContext) -> CompletableFuture<HttpContext> {
    ...
    override fun invoke(context: HttpContext): CompletableFuture<HttpContext> {
        ...
        return try {
            ...
            context.ok(bytes, contentType = mediaType?.let { ContentType(it) }).done()
        }
        catch (e: ResourceNotFoundException) {
            context.notFound(e.message ?: "").done()
        }
    }
}
