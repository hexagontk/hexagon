...
internal class HttpContextTest {
    ...
    @Test fun `Response helpers return correct values`() {
        val path = PathHandler(
            OnHandler("/ok") { ok().done() },
            OnHandler("/notFound") { notFound().done() },
            OnHandler("/noContent") { send(HttpResponse(status = NO_CONTENT_204)).done() },
            OnHandler("/accepted") { send(status = ACCEPTED_202).done() },
            OnHandler("/created") { created().done() },
            OnHandler("/badRequest") { badRequest().done() },
            OnHandler("/clientError") { send(FORBIDDEN_403).done() },
            OnHandler("/success") { send(NO_CONTENT_204).done() },
            OnHandler("/serverError") { send(BAD_GATEWAY_502).done() },
            OnHandler("/redirect") { send(FOUND_302).done() },
            OnHandler("/internalServerError") { internalServerError().done() },
            OnHandler("/internalServerErrorException") { internalServerError(RuntimeException()).done() },
            OnHandler("/serverErrorException") { serverError(BAD_GATEWAY_502, RuntimeException()).done() },
        )

        assertEquals(OK_200, path.process(HttpRequest(path = "/ok")).join().status)
        assertEquals(NOT_FOUND_404, path.process(HttpRequest(path = "/notFound")).join().status)
        assertEquals(NO_CONTENT_204, path.process(HttpRequest(path = "/noContent")).join().status)
        assertEquals(ACCEPTED_202, path.process(HttpRequest(path = "/accepted")).join().status)
        assertEquals(CREATED_201, path.process(HttpRequest(path = "/created")).join().status)
        assertEquals(BAD_REQUEST_400, path.process(HttpRequest(path = "/badRequest")).join().status)
        assertEquals(FORBIDDEN_403, path.process(HttpRequest(path = "/clientError")).join().status)
        assertEquals(NO_CONTENT_204, path.process(HttpRequest(path = "/success")).join().status)
        assertEquals(BAD_GATEWAY_502, path.process(HttpRequest(path = "/serverError")).join().status)
        assertEquals(FOUND_302, path.process(HttpRequest(path = "/redirect")).join().status)
        assertEquals(
            INTERNAL_SERVER_ERROR_500,
            path.process(HttpRequest(path = "/internalServerError")).join().status
        )

        path.process(HttpRequest(path = "/internalServerErrorException")).join().response.let {
        ...
        path.process(HttpRequest(path = "/serverErrorException")).join().response.let {
    ...
    @Test fun `'next' executes the next handler in the chain`() {
        ...
        assertEquals(OK_200, context.nextContext().join().response.status)
    }
    ...
    @Test fun `Context can change the request`() {
        val path = PathHandler(
            OnHandler("*") { request(body = request.bodyString() + "_modified").done() },
            OnHandler("/test") { ok(body = request.body).done() },
        )

        path.process(HttpRequest(path = "/test")).join().apply {
            assertEquals(OK_200, status)
            assertEquals("_modified", response.bodyString())
        }

        path.process(HttpRequest(path = "/test", body = "body")).join().apply {
            assertEquals(OK_200, status)
