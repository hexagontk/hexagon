...
internal class FileCallbackTest {
    ...
    @Test fun `Filter with one parameter resolves to a file`() = runBlocking {
    ...
    @Test fun `Filter with one parameter resolves to a missing file`() = runBlocking {
    ...
    @Test fun `Filter with no parameter resolves to the callback's file`() = runBlocking {
    ...
    @Test fun `Return file without known media type`() = runBlocking {
    ...
    @Test fun `Return file with known media type`() = runBlocking {
    ...
    private suspend fun processFileCallback(
        filePath: String,
        requestPath: String,
        pathPattern: String
    ): HttpResponsePort =
