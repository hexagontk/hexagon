...
internal class SerializeResponseCallbackTest {
    ...
    @Test fun `Serialize response callback creates the proper response`() {
        ...
        assertEqualHttpRequests(
            jsonContext.send(body = jsonBody).request,
            callback(jsonContext).request)
        assertEqualHttpResponses(
            jsonContext.send(body = jsonBody).response,
            callback(jsonContext).response
        )
        ...
        assertEqualHttpRequests(expectedContext.request, callback(acceptContext).request)
        assertEqualHttpResponses(expectedContext.response, callback(acceptContext).response)
    }
}
