...
typealias HttpCallback = suspend HttpContext.() -> HttpContext
typealias HttpExceptionCallback<T> = suspend HttpContext.(T) -> HttpContext

internal fun toCallback(block: HttpCallback): suspend (Context<HttpCall>) -> Context<HttpCall> =
    { context -> HttpContext(context).block() }

suspend fun HttpCallback.process(
    request: HttpRequest,
