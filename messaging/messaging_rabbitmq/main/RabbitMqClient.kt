...
class RabbitMqClient(
    ...
    internal companion object {
        ...
        internal fun createConnectionFactory(uri: URI): ConnectionFactory {
            ...
            val params = queryParameters.filterNot { (_, v) -> v.text.isBlank() }
            fun value(name: String): String? = params[name]?.text
    ...
    fun <T : Any> consume(
        ...
        consume(routingKey, type, decoder, handler)
    }

    fun <T : Any, R : Any> consume(
        queueName: String, type: KClass<T>, decoder: (Map<String, *>) -> T, handler: (T) -> R
    ) {
